...
class BandAidItem(settings: Properties) : Item(settings) {
	override fun addInformation(
		...
		tooltip.add(TranslationTextComponent("text." + HealthMod.MOD_ID + ".band_aid.1"))
		tooltip.add(TranslationTextComponent("text." + HealthMod.MOD_ID + ".band_aid.2"))
		tooltip.add(TranslationTextComponent("text." + HealthMod.MOD_ID + ".band_aid.3"))
	}
		...
		if (!world.isRemote) {
			...
			if (player.maxHealth > player.health) {
				...
				val chance = if (player.getActivePotionEffect(EffectRegistries.HEALTHY) != null) {
					HealthModConfig.SERVER_CONFIG.bandAidInfectionChanceWhenHealthy.get()
				} else {
					HealthModConfig.SERVER_CONFIG.bandAidInfectionChance.get()
				}
				...
				if (ThreadLocalRandom.current().nextInt(1, chance + 1) == 1) {
					...
					player.sendStatusMessage(
						TranslationTextComponent("text." + HealthMod.MOD_ID + ".band_aid.failed_apply"),
						true
					)
